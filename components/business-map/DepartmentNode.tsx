...
import { Users, Zap, GitBranch, X, CheckCircle2, AlertTriangle, ChevronDown, ChevronUp, Loader2 } from "lucide-react";
...
function impactConfig(type: string) {
  ...
  return map[type] ?? { label: type, color: "var(--bv-text-3)", bg: "var(--bv-text-3)15" };
}
  ...
  return (
    <div className="relative flex items-center justify-center shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute flex flex-col items-center leading-none">
        ...
        <span style={{ fontSize: size > 64 ? 9 : 7, color: "var(--bv-muted)", fontFamily: "var(--font-inter)", marginTop: 1 }}>{label}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: "hidden",
        background: isRoot
          ? `linear-gradient(160deg, #1a1d2b 0%, var(--bv-surface) 100%)`
          : "linear-gradient(160deg, #16182188 0%, var(--bv-surface) 100%)",
        boxShadow: isRoot
    ...
    >
      ...
      <div style={{ padding: isRoot ? "16px 18px 14px" : "14px 16px 12px" }}>
            ...
            <div className="min-w-0">
              ...
              {data.headcount !== null && (
                <div className="flex items-center gap-1 mt-0.5">
                  <Users style={{ width: 9, height: 9, color: "var(--bv-muted)" }} strokeWidth={2} />
                  <span style={{ fontSize: 9, color: "var(--bv-muted)", fontFamily: "var(--font-inter)" }}>{data.headcount} אנשים</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div style={{ height: 1, backgroundColor: "var(--bv-border)", marginBottom: 10 }} />

        {/* Stat badges */}
        <div className="flex items-center gap-2 relative z-10">
          <div className="flex items-center gap-1.5 flex-1 rounded-lg px-2.5 py-1.5 backdrop-blur-md" style={{ backgroundColor: "#1a1d2790", border: "1px solid var(--bv-border)" }}>
            <GitBranch style={{ width: 10, height: 10, color: data.color || "var(--bv-text-3)" }} strokeWidth={2} />
            <div>
              <div style={{ fontSize: isRoot ? 15 : 13, fontWeight: 700, color: "var(--bv-text-1)", lineHeight: 1, fontFamily: "var(--font-inter)" }}>{animP}</div>
              <div style={{ fontSize: 8, color: "var(--bv-muted)", fontFamily: "var(--font-inter)", marginTop: 1 }}>תהליכים</div>
            </div>
            {manualCount > 0 && (
          ...
          <div className="flex items-center gap-1.5 flex-1 rounded-lg px-2.5 py-1.5 backdrop-blur-md"
            style={{ backgroundColor: data.opportunityCount > 0 ? "#4d8eff20" : "#1a1d2790", border: data.opportunityCount > 0 ? "1px solid #4d8eff40" : "1px solid var(--bv-border)" }}>
            <Zap style={{ width: 10, height: 10, color: data.opportunityCount > 0 ? "#4d8eff" : "var(--bv-muted)" }} strokeWidth={2} />
            <div>
              <div style={{ fontSize: isRoot ? 15 : 13, fontWeight: 700, color: data.opportunityCount > 0 ? "#4d8eff" : "var(--bv-muted)", lineHeight: 1, fontFamily: "var(--font-inter)" }}>{animO}</div>
              <div style={{ fontSize: 8, color: "var(--bv-muted)", fontFamily: "var(--font-inter)", marginTop: 1 }}>הזדמנויות</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 flex items-center justify-end">
          <span style={{ fontSize: 9, color: "var(--bv-muted)", fontFamily: "var(--font-inter)" }}>לחץ להגדלה →</span>
        </div>
      </div>
    </div>
  );
...
function ExpandedCard({ data, score, healthStroke, healthGlow, emoji, addedTasks, onTaskAdded }: {
  ...
  async function toggleProcess(process: any) {
    ...
    try {
      ...
      setBreakdownCache((prev) => ({ ...prev, [process.id]: breakdownData }));
    } catch (err) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 20,
        overflow: "hidden",
        background: "linear-gradient(160deg, #191c28 0%, var(--bv-bg) 100%)",
    ...
    >
      ...
      {/* Header */}
      <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid var(--bv-border)" }}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <h2 className="truncate" style={{ fontSize: 19, fontWeight: 800, color: "var(--bv-text-1)", fontFamily: "var(--font-manrope)", letterSpacing: "-0.02em" }}>
                  {data.label}
                </h2>
              </div>
              {data.headcount !== null && (
                <div className="flex items-center gap-1 mt-1">
                  <Users style={{ width: 10, height: 10, color: "var(--bv-muted)" }} strokeWidth={2} />
                  <span style={{ fontSize: 10, color: "var(--bv-muted)", fontFamily: "var(--font-inter)" }}>{data.headcount} אנשים</span>
                </div>
              )}
              <div className="flex items-center gap-3 mt-1.5">
                <span style={{ fontSize: 10, color: "var(--bv-text-3)", fontFamily: "var(--font-inter)" }}>
                  <span style={{ color: "var(--bv-text-1)", fontWeight: 700 }}>{data.processes.length}</span> תהליכים
                </span>
                <span style={{ color: "var(--bv-muted)", fontSize: 10 }}>·</span>
                <span style={{ fontSize: 10, color: "var(--bv-text-3)", fontFamily: "var(--font-inter)" }}>
                  <span style={{ color: "#4d8eff", fontWeight: 700 }}>{data.opportunityCount}</span> הזדמנויות AI
                </span>
              </div>
              <div style={{ marginTop: 4, fontSize: 10, color: "var(--bv-text-3)", fontFamily: "var(--font-inter)" }}>
                {healthDesc}
              </div>
            </div>
          </div>
          ...
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "var(--bv-border)"; e.currentTarget.style.color = "#f87171"; }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "var(--bv-surface-elevated)"; e.currentTarget.style.color = "var(--bv-text-3)"; }}
          >
            <X size={13} />
          </button>
        </div>
      ...
      >
          ...
          <div className="mb-6 flex flex-col gap-2">
            {data.mainPain && (
              <div style={{ backgroundColor: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.15)", borderRadius: 8, padding: "8px 12px" }}>
                ...
                <div style={{ fontSize: 12, color: "var(--bv-text-1)", lineHeight: 1.4 }}>{data.mainPain}</div>
              </div>
            )}
            {data.firstAction && (
              <div style={{ backgroundColor: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.15)", borderRadius: 8, padding: "8px 12px" }}>
                ...
                <div style={{ fontSize: 12, color: "var(--bv-text-1)", lineHeight: 1.4 }}>{data.firstAction}</div>
              </div>
            )}
          </div>
        )}
        ...
        <section style={{ marginBottom: 20 }}>
          <div className="flex items-center gap-2 mb-2">
            <GitBranch size={11} style={{ color: data.color || "var(--bv-text-3)" }} />
            <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.12em", color: "var(--bv-muted)", textTransform: "uppercase", fontFamily: "var(--font-inter)" }}>
              תהליכים ומשימות
            </span>
          </div>

          {data.processes.length === 0 ? (
            <p style={{ fontSize: 11, color: "var(--bv-border-subtle)", fontStyle: "italic" }}>אין תהליכים עדיין</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {data.processes.map((p) => {
                ...
                return (
                  <div key={p.id} className="flex flex-col gap-0 rounded-lg overflow-hidden border transition-colors duration-200"
                    style={{ backgroundColor: "var(--bv-surface-elevated)", borderColor: isExpanded ? "var(--bv-border)" : "var(--bv-border)" }}>
                    <button 
                      onClick={(e) => { e.stopPropagation(); toggleProcess(p); }}
                      className="flex items-center justify-between gap-2 w-full text-right"
                      style={{ padding: "7px 11px", cursor: "pointer", background: "transparent", border: "none" }}
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        {isExpanded ? <ChevronUp size={12} style={{ color: "var(--bv-text-3)", flexShrink: 0 }} /> : <ChevronDown size={12} style={{ color: "var(--bv-text-3)", flexShrink: 0 }} />}
                        {p.isManual
                          ? <AlertTriangle size={10} style={{ color: "#fbbf24", flexShrink: 0 }} />
                          : <CheckCircle2 size={10} style={{ color: "#34d399", flexShrink: 0 }} />
                        }
                        <span style={{ fontSize: 11, color: "var(--bv-text-2)", fontFamily: "var(--font-inter)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {p.name}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        {p.frequency && (
                          <span style={{ fontSize: 8, color: "var(--bv-muted)", backgroundColor: "var(--bv-border)", padding: "1px 6px", borderRadius: 999, fontFamily: "var(--font-inter)" }}>
                            {p.frequency}
                          </span>
                        )}
                        <span style={{
                    ...
                    >
                      {isExpanded && (
                        <div style={{ padding: "10px 11px 14px", borderTop: "1px solid var(--bv-border)" }}>
                          {isLoading ? (
                            <div className="flex flex-col items-center justify-center py-4 gap-2">
                              <Loader2 size={14} className="animate-spin" style={{ color: "#4d8eff" }} />
                              <span style={{ fontSize: 10, color: "var(--bv-text-3)", fontFamily: "var(--font-inter)" }}>מנתח תהליך...</span>
                            </div>
                          ) : breakdown ? (
                            <div className="space-y-3">
                              {breakdown.subSteps.map((step: any, idx: number) => (
                                <div key={idx} className="flex gap-2">
                                  <div className="shrink-0 flex items-center justify-center w-4 h-4 rounded-full mt-0.5" style={{ backgroundColor: "var(--bv-border)", color: "var(--bv-text-3)", fontSize: 9, fontWeight: 800 }}>
                                    {idx + 1}
                                  </div>
                                  <div className="min-w-0 flex-1">
                                    <div className="flex items-start justify-between gap-2 mb-1">
                                      <span style={{ fontSize: 10, color: "var(--bv-text-1)", fontWeight: 700, fontFamily: "var(--font-inter)", lineHeight: 1.3 }}>{step.name}</span>
                                      <span style={{ 
                                        fontSize: 8, fontWeight: 700, padding: "2px 6px", borderRadius: 4, whiteSpace: "nowrap",
                                    ...
                                    <p style={{ fontSize: 9, color: "var(--bv-text-3)", lineHeight: 1.4, fontFamily: "var(--font-inter)" }}>
                                      {step.description}
                                    </p>
                                    {step.automatable && step.tool && (
                                      <div className="mt-1.5 flex items-center gap-1" style={{ backgroundColor: "rgba(251,191,36,0.1)", padding: "2px 6px", borderRadius: 4, width: "fit-content" }}>
                                        ...
                                        <span style={{ fontSize: 8, color: "#fbbf24", fontFamily: "var(--font-inter)", fontWeight: 600 }}>{step.tool}</span>
                                      </div>
                                    )}
                                  </div>
                                </div>
                            ...
                            <div className="text-center py-2">
                              <span style={{ fontSize: 10, color: "#f87171" }}>שגיאה בטעינת הנתונים</span>
                            </div>
                          )}
                        </div>
                      )}
        ...
        <section>
          ...
          {data.aiOpportunities.length === 0 ? (
            <p style={{ fontSize: 11, color: "var(--bv-border-subtle)", fontStyle: "italic" }}>הפעל ניתוח AI לקבלת המלצות</p>
          ) : (
                ...
                return (
                  <div key={opp.id} style={{ backgroundColor: "#4d8eff08", border: "1px solid #4d8eff20", borderRadius: 10, padding: "10px 12px" }}>
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <span style={{ fontSize: 12, fontWeight: 600, color: "var(--bv-text-1)", fontFamily: "var(--font-manrope)", lineHeight: 1.4, flex: 1 }}>
                        {opp.title}
                      </span>
                    ...
                    {(() => {
                      ...
                      const btnColor = isAlreadyExisting ? "var(--bv-text-3)" : isJustAdded ? "#34d399" : (creatingTask === opp.id ? "var(--bv-muted)" : "#4d8eff");
      ...
      {toastData && typeof document !== "undefined" && createPortal(
        <div style={{
          ...
          backgroundColor: "#191c28", border: "1px solid var(--bv-border)", borderRadius: 12,
          ...
          <div style={{ fontSize: 13, color: "var(--bv-text-1)", marginBottom: 6, fontWeight: 500, lineHeight: 1.4 }}>{toastData.title}</div>
          <div style={{ fontSize: 11, color: "var(--bv-text-3)", marginBottom: 12 }}>זמן משוער: {toastData.hours} שעות</div>
